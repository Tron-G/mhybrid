...
function drawBar(container, data, max_data, min_data) {
  ...
  const bar_title = ["time", "cost", "distance", "carbon"];
  ...
  for (let i = 0; i < 4; i++) {
    ...
    let x_pos = rect_side + i * (rect_width + rect_margin) + rect_width / 2;
    let fonSize = 15,
      fontColor = "gray";
    if (i >= 2) {
      fonSize = 12;
    ...
    svg
      .append("text")
      .attr("fill", fontColor)
      .attr("font-size", fonSize + "px")
      .attr("text-anchor", "middle")
      .attr("font-weight", "bold")
      .attr("x", x_pos)
      .attr("y", 20)
      .text(bar_title[i]);
    // .attr("transform", "rotate(-25) translate(0," + 20 * i + ")");
...
function sortItem(data, now_sort, last_sort) {
  ...
  for (let i = 0; i < now_index.length; i++) {
    if (now_index[i] != last_index[i]) {
      ...
      d3.select("#result_item" + now_index[i])
        .transition()
        .duration(500)
        .style("left", move + "px");
    }
...
function drawRouteInfo(data, container) {
  ...
  for (let i = 0; i < data["route_street"].length; i++) {
    ...
    if (i == 0) {
      tips = "get on a ";
      if (data["route_mode"][i] == 0) tips += "bike";
      else tips += "taxi";
    ...
    if (i < data["route_street"].length - 1 && data["route_mode"][i] == 0) mode_img = bike_src;
    ...
    if (i < data["route_street"].length - 1) {
      let path_info =
        data["route_carbon_list"][i].toFixed(2) +
        "kg , " +
        data["route_distance"][i] +
        "m" +
        " , " +
        data["route_time"][i] +
        "s";
...
export { drawBar, removeAllSvg, sortItem, resetItemPos, drawRouteInfo, removeSvg };
