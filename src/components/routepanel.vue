<template>
  <div>
    <div id="route_panel">
      ...
      <div id="switch_bt" class="btns switching" @click="switchView()" :class="{ is_line_view: now_view == 'line' }">
        {{ switch_btn_text }}
      </div>
      <div id="sort_tip">sort by:</div>
      <div id="sort_panel">
        <div id="time_bt" class="btns sort_bt" @click="sortResult('time')" :class="{ active: now_sort == 'time' }">
          time
        </div>
        <div id="cost_bt" class="btns sort_bt" @click="sortResult('cost')" :class="{ active: now_sort == 'cost' }">
          cost
        </div>
        <div
          id="distance_bt"
      ...
      <div id="result_view">
        <div
          ...
          class="route_item"
          :id="getItemId(index)"
          v-show="show_view('bar')"
        >
          <div class="route_attr_chart" :id="getViewId(index)"></div>
          <div class="info">route index: {{ index + 1 }}</div>
          <div class="btn_area">
            <div class="route_bt btns" :id="getChooseId(index)" @click="chooseRoute(index)" :class="isActive(index)">
              show
            </div>
            <div
              class="route_bt btns"
        ...
        <div v-show="show_view('line')" id="bar_line_chart"></div>
      </div>
    </div>
  </div>
</template>
...
import { drawLineBar } from "assets/js/drawEcharts";
// 展示候选路线的详细信息以及提供选择交互
export default {
  name: "routepanel",
  components: {},
  ...
  mounted() {
    // console.log("@@@@", this.route_attr);
  },
  computed: {
    switch_btn_text() {
      return this.now_view === "bar" ? "line" : "bar";
    },
  },
  methods: {
    //计算各个路线板块下的按钮id
    ...
    //转换柱状图和折线图
    switchView() {
      if (this.now_view === "bar") {
        this.now_view = "line";
        this.$nextTick(() => {
          drawLineBar("bar_line_chart", this.route_attr);
        });
      } else this.now_view = "bar";
    },
    //控制面板切换
    show_view(name) {
      return name === this.now_view;
    },
    show() {
      let route = document.getElementById("route_panel");
      if (route.classList.contains("hide_window") || route.classList.length == 0) {
    ...
    drawBar() {
      ...
      for (let i = 0; i < data[0].length; i++) d3drawer.drawBar(this.getViewId(i), data[0][i], data[1], data[2]);
    },
    ...
    sortResult(sort_type) {
      ...
      let new_index = d3drawer.sortItem(this.route_attr, sort_type, this.now_sort);
    ...
    dataTransfer() {
      ...
      let max_data = [Math.max(...time), Math.max(...cost), Math.max(...distance), Math.max(...carbon)];
      let min_data = [Math.min(...time), Math.min(...cost), Math.min(...distance), Math.min(...carbon)];
      for (let i = 0; i < time.length; i++) res.push([time[i], cost[i], distance[i], carbon[i]]);
